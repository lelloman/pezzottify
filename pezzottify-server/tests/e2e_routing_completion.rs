...
async fn upload_preserves_auth_extraction_and_field_errors() {
    ...
    let missing_type=admin.client.post(&url).body("not multipart").send().await.unwrap();
...
async fn unsupported_methods_and_head_preserve_router_contracts() {
    ...
    let missing=admin.client.get(format!("{}/v1/no-such-route",server.base_url)).send().await.unwrap();
