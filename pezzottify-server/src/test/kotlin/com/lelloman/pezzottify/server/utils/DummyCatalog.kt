    ...
    companion object {
        fun create(httpClient: HttpClient): DummyCatalog {
            ...
            val individuals = IntArray(10) { it }.map { index ->
                ...
                httpClient.createArtist(artist)
                    .addFile("image", mockPng(100 + index, 100 + index))
                    .execute()
                    .assertStatus2xx()
            ...
            val band2 = httpClient.createArtist(
                CreateBandRequest(
                    "Band 2",
                    listOf(individuals[3].id, individuals[4].id, individuals[5].id)
                )
            )
                .addFile("image", mockPng())
                .execute()
            ...
            val album1: Album = httpClient.multipartPost("/api/album")
                .addJsonField("album", album1Request)
                .addFiles("audioTracks", album1Request.audioTracksNames, listOf(MP3_SAMPLE, MP3_SAMPLE))
            ...
            val album2: Album = httpClient.multipartPost("/api/album")
                .addJsonField("album", album2Request)
                .addFiles("audioTracks", album2Request.audioTracksNames, listOf(MP3_SAMPLE, MP3_SAMPLE))
                .addFile("cover", mockPng())
                .execute()
                .assertStatus2xx()
                .parsedBody()
