...
/// Configuration for track audio embeddings generated by Simple-AI.
#[derive(Debug, Deserialize, Default, Clone)]
#[serde(default)]
pub struct AudioEmbeddingsConfig {
    /// Whether the track embedding synchronizer is enabled.
    pub enabled: Option<bool>,
    /// Simple-AI base URL, for example "http://simple-ai:8080".
    pub simple_ai_base_url: Option<String>,
    /// Literal API key. Prefer api_key_env for deployed configs.
    pub api_key: Option<String>,
    ...
    /// Number of tracks to process per scheduled run.
    pub max_tracks_per_run: Option<usize>,
    /// HTTP request timeout for each Simple-AI call.
