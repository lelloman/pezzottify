...
impl Fts5LevenshteinSearchVault {
    ...
    pub fn with_max_distance(
        ...
        // Build vocabulary and index content
        ...
        debug!(
            "FTS5+Levenshtein search index built with {} items, vocabulary has {} words",
            count,
            vocabulary.len()
        );
...
impl SearchVault for Fts5LevenshteinSearchVault {
    ...
    fn add_item(&self, id: &str, item_type: HashedItemType, name: &str) {
        ...
        if let Err(e) = conn.execute(
            ...
            warn!("FTS5+Levenshtein add_item failed: {}", e);
            return;
        }
    ...
    fn update_item(&self, id: &str, item_type: HashedItemType, name: &str) {
        ...
        if let Err(e) = conn.execute(
            ...
            warn!("FTS5+Levenshtein update_item delete failed: {}", e);
            return;
        }
        if let Err(e) = conn.execute(
            "INSERT INTO search_index (item_id, item_type, name) VALUES (?, ?, ?)",
            ...
            warn!("FTS5+Levenshtein update_item insert failed: {}", e);
            return;
        }

        // Add new words to vocabulary (we don't remove old words as they might
        // still be used by other items - vocabulary grows but doesn't shrink)
        let mut vocabulary = self.vocabulary.write().unwrap();
        vocabulary.add_text(name);
    }
    ...
    fn remove_item(&self, id: &str, item_type: HashedItemType) {
        ...
        if let Err(e) = conn.execute(
            ...
            warn!("FTS5+Levenshtein remove_item failed: {}", e);
        }
        // Note: We don't remove words from vocabulary as they might still be
        // referenced by other items. The vocabulary can only grow.
    }
}
...
mod tests {
    ...
    #[test]
    fn test_add_updates_vocabulary() {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("search.db");
        ...
        // Add item
        vault.add_item("new1", HashedItemType::Artist, "Nirvana");

        // Search with typo should now work
        let results = vault.search("Nirvna", 10, None);
        assert_eq!(results.len(), 1);
