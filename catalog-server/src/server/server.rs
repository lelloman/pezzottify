...
pub async fn make_app(
    ...
    // Initialize download manager if enabled (queue works even without QT configured)
    if config.download_manager.enabled {
        ...
        match crate::download_manager::SqliteDownloadQueueStore::new(&queue_db_path) {
            Ok(queue_store) => {
                ...
                let torrent_client_dyn: Option<
                    Arc<dyn crate::download_manager::TorrentClientTrait>,
                > = torrent_client
                    .clone()
